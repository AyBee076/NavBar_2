        ...
        <div className="w-1/2 flex flex-col px-5 md:px-8 ">
          ...
          <div className="mt-4">
            ...
            <div className="mt-2 flex gap-2">
              {model.size.map((item) => (
                <Button variant="outline" className="bg-gray-300" key={item}>
                  {item}
                </Button>
              ))}
            </div>
          ...
          {/* // Quantity selector + stock indicator JSX: */}
          {/* STOCK INDICATOR */}
          <div className="mt-4">
            {model.stock > 0 ? (
              <p
                className={`text-sm font-medium ${model.stock <= 5 ? "text-orange-500" : "text-green-600"}`}
              >
                {model.stock <= 5
